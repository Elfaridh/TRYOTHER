...
export async function setupLayout(active){
  ...
  if(nav){
    const items=[['index.html','Dashboard'],['lokasi.html','Lokasi'],['aset.html','Aset'],['pemeliharaan.html','Pemeliharaan'],['mutasi.html','Mutasi'],['laporan.html','Laporan']];
    nav.innerHTML=items.map(([h,t])=>`<a href="${h}" class="${active===h?'active':''}">${t}</a>`).join('')+`<a href="#" id="logout-btn">Logout</a>`;
